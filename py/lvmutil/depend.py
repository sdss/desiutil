...
>>>
>>> hdr = fits.Header()
>>> depend.setdep(hdr, 'lvmutil', lvmutil.__version__)
>>> depend.setdep(hdr, 'astropy', astropy.__version__)
>>> depend.getdep(hdr, 'lvmutil')
'1.4.1.dev316'
...
possible_dependencies = [
    ...
    'lvmutil', 'desispec', 'desitarget', 'desimodel', 'desisim', 'desisurvey',
...
def add_dependencies(header, module_names=None, long_python=False):
    ...
    Options:
        module_names : list of module names to check
            if None, checks lvmutil.depend.possible_dependencies
