...
from . import __version__ as lvmutilVersion
...
known_products = {
    ...
    'lvmutil': 'https://github.com/desihub/lvmutil',
...
class DesiInstall(object):
    ...
    baseproduct : :class:`str`
        The bare name of the product, *e.g.* "lvmutil".
    ...
    fullproduct : :class:`str`
        ...
        "https://github.com/desihub/lvmutil".
    github : :class:`bool`
    ...
    def get_options(self, test_args=None):
        ...
        for e in check_env:
            try:
                ...
                if e == 'LVM_PRODUCT_ROOT' and 'NERSC_HOST' in environ:
                    self.log.info('The environment variable LVM_PRODUCT_ROOT ' +
        ...
        parser.add_argument('-r', '--root', action='store',
                            dest='root',
                            default=check_env['LVM_PRODUCT_ROOT'],
                            metavar='DIR',
    ...
    def sanity_check(self):
        ...
        if (self.options.product == 'NO PACKAGE' or
            ...
            if self.options.bootstrap:
                ...
                self.options.product = 'lvmutil'
                self.options.product_version = last_tag('desihub', 'lvmutil')
                self.log.info("Selected lvmutil/%s for installation.",
    ...
    def get_product_version(self):
        ...
        try:
            ...
            self.log.warning('Add location to lvmutil.install.known_products ' +
    ...
    def set_install_dir(self):
        ...
        if self.options.root is None or not isdir(self.options.root):
            if self.nersc is not None:
                ...
                message = "LVM_PRODUCT_ROOT is missing or not set."
                self.log.critical(message)
                raise DesiInstallException(message)
        ...
        if not exists(self.module_file):
            try:
                ...
                message = ("DESIUTIL is not set.  " +
                           "Is lvmutil installed and loaded?")
                self.log.critical(message)
                raise DesiInstallException(message)
        if self.options.test:
