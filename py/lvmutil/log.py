...
3. Change the default log level on the command-line.  This can actually be
   ...
   :func:`~lvmutil.log.get_logger` with the appropriate level, or
   the environment variable :envvar:`LVM_LOGLEVEL` can be set.
...
>>> from lvmutil.log import get_logger, DesiLogContext, DEBUG
...
def _configure_root_logger(timestamp=False, delimiter=':'):
    ...
    """
    root_name = "lvmutil.log.dlm" + ''.join(map(str, map(ord, delimiter)))
    if timestamp:
        root_name += 'timestamp'
    if root_name not in _lvmutil_log_root:
        ...
        _lvmutil_log_root[root_name] = logging.getLogger(root_name)
        _lvmutil_log_root[root_name].addHandler(ch)
        _lvmutil_log_root[root_name].setLevel(logging.INFO)
    return root_name
